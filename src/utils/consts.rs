use crate::utils::enums::{FrameRestorationType, RefFrame};
...
// Number of each of left and above contexts for intra_FRAME_y_mode
...
// Maximum times the transform can be split
...
pub const SUPERRES_FILTER_SHIFTS: u32 = 1 << SUPERRES_FILTER_BITS;
...
pub const LAST_FRAME: usize = RefFrame::Last as usize;



pub const SEGMENTATION_FEATURE_BITS: [u8; SEG_LVL_MAX] = [8, 6, 6, 6, 6, 3, 0, 0];
pub const SEGMENTATION_FEATURE_SIGNED: [u8; SEG_LVL_MAX] = [1, 1, 1, 1, 1, 0, 0, 0];
pub const SEGMENTATION_FEATURE_MAX: [u8; SEG_LVL_MAX] = [255, MAX_LOOP_FILTER, MAX_LOOP_FILTER, MAX_LOOP_FILTER, MAX_LOOP_FILTER, 7, 0, 0];


pub const RESTORE_SWITCHABLE: usize = FrameRestorationType::Switchable as usize;


// 9.4. Default CDF tables
pub const DEFAULT_INTRA_FRAME_Y_MODE_CDF: [[[u32; INTRA_MODES + 1]; INTRA_MODE_CONTEXTS]; INTRA_MODE_CONTEXTS] = [
    [
...
pub const DEFAULT_Y_MODE_CDF: [[u32; INTRA_MODES + 1]; BLOCK_SIZE_GROUPS] = [
...
pub const DEFAULT_UV_MODE_CFL_NOT_ALLOWED_CDF: [[u32; UV_INTRA_MODES_CFL_NOT_ALLOWED + 1]; INTRA_MODES] = [
...
pub const DEFAULT_UV_MODE_CFL_ALLOWED_CDF: [[u32; UV_INTRA_MODES_CFL_ALLOWED + 1]; INTRA_MODES] = [
...
pub const DEFAULT_ANGLE_DELTA_CDF: [[u32; (2 * MAX_ANGLE_DELTA + 1) + 1]; DIRECTIONAL_MODES]= [
...
pub const DEFAULT_INTRABC_CDF: [u32; 2 + 1] = [30531, 32768, 0];

pub const DEFAULT_PARTITION_W8_CDF: [[u32; 5]; PARTITION_CONTEXTS] = [
...
pub const DEFAULT_PARTITION_W16_CDF: [[u32; 11]; PARTITION_CONTEXTS] = [
...
pub const DEFAULT_PARTITION_W32_CDF: [[u32; 11]; PARTITION_CONTEXTS] = [
...
pub const DEFAULT_PARTITION_W64_CDF: [[u32; 11]; PARTITION_CONTEXTS] = [
...
pub const DEFAULT_PARTITION_W128_CDF: [[u32; 9]; PARTITION_CONTEXTS] = [
...
pub const DEFAULT_TX_8X8_CDF: [[u32; MAX_TX_DEPTH + 1]; TX_SIZE_CONTEXTS ] = [
    [ 19968, 32768, 0 ],
    [ 19968, 32768, 0 ],
    [ 24320, 32768, 0 ]
];

pub const DEFAULT_TX_16X16_CDF: [[u32; MAX_TX_DEPTH + 2]; TX_SIZE_CONTEXTS ] = [
    [ 12272, 30172, 32768, 0 ],
    [ 12272, 30172, 32768, 0 ],
    [ 18677, 30848, 32768, 0 ]
];

pub const DEFAULT_TX_32X32_CDF: [[u32; MAX_TX_DEPTH + 2]; TX_SIZE_CONTEXTS ] = [
    [ 12986, 15180, 32768, 0 ],
    [ 12986, 15180, 32768, 0 ],
    [ 24302, 25602, 32768, 0 ]
];

pub const DEFAULT_TX_64X64_CDF: [[u32; MAX_TX_DEPTH + 2]; TX_SIZE_CONTEXTS ] = [
    [ 5782, 11475, 32768, 0 ],
    [ 5782, 11475, 32768, 0 ],
    [ 16803, 22759, 32768, 0 ]
];

pub const DEFAULT_TXFM_SPLIT_CDF: [[u32; 3]; TXFM_PARTITION_CONTEXTS ] = [
...
pub const DEFAULT_FILTER_INTRA_MODE_CDF: [u32; 6] = [ 8949, 12776, 17211, 29558, 32768, 0 ];

pub const DEFAULT_FILTER_INTRA_CDF: [[u32; 3]; BLOCK_SIZES] = [
...
pub const DEFAULT_SEGMENT_ID_CDF: [[u32; MAX_SEGMENTS + 1]; SEGMENT_ID_CONTEXTS] = [
...
pub const DEFAULT_SEGMENT_ID_PREDICTED_CDF: [[u32; 3]; SEGMENT_ID_PREDICTED_CONTEXTS] = [
  [ 128 * 128, 32768, 0 ],
  [ 128 * 128, 32768, 0 ],
  [ 128 * 128, 32768, 0 ]
];

pub const DEFAULT_MV_CLASS0_HP_CDF: [u32; 3] = [
  160*128, 32768, 0
];

pub const DEFAULT_MV_HP_CDF: [u32; 3] = [
  128*128, 32768, 0
];

pub const DEFAULT_MV_SIGN_CDF: [u32; 3] = [
  128*128, 32768, 0
];

pub const DEFAULT_MV_BIT_CDF: [[u32; 3]; MV_OFFSET_BITS] = [
  [ 136*128, 32768, 0 ],
  [ 140*128, 32768, 0 ],
  [ 148*128, 32768, 0 ],
  [ 160*128, 32768, 0 ],
...
pub const DEFAULT_MV_CLASS0_BIT_CDF: [u32; 3] = [
  216*128, 32768, 0
];

pub const DEFAULT_NEW_MV_CDF: [[u32; 3]; NEW_MV_CONTEXTS] = [
  [ 24035, 32768, 0 ],
  [ 16630, 32768, 0 ],
  [ 15339, 32768, 0 ],
  [ 8386, 32768, 0 ],
...
pub const DEFAULT_ZERO_MV_CDF: [[u32; 3]; ZERO_MV_CONTEXTS] = [
  [ 2175, 32768, 0 ],
  [ 1054, 32768, 0 ]
];

pub const DEFAULT_REF_MV_CDF: [[u32; 3]; REF_MV_CONTEXTS] = [
  [ 23974, 32768, 0 ],
  [ 24188, 32768, 0 ],
  [ 17848, 32768, 0 ],
  [ 28622, 32768, 0 ],
...
pub const DEFAULT_DRL_MODE_CDF: [[u32; 3]; DRL_MODE_CONTEXTS] = [
  [ 13104, 32768, 0 ],
  [ 24560, 32768, 0 ],
  [ 18945, 32768, 0 ]
];

pub const DEFAULT_IS_INTER_CDF: [[u32; 3]; IS_INTER_CONTEXTS] = [
  [ 806, 32768, 0 ],
  [ 16662, 32768, 0 ],
  [ 20186, 32768, 0 ],
  [ 26538, 32768, 0 ]
...
pub const DEFAULT_COMP_MODE_CDF: [[u32; 3]; COMP_INTER_CONTEXTS] = [
  [ 26828, 32768, 0 ],
  [ 24035, 32768, 0 ],
  [ 12031, 32768, 0 ],
  [ 10640, 32768, 0 ],
...
pub const DEFAULT_SKIP_MODE_CDF: [[u32; 3]; SKIP_MODE_CONTEXTS] = [
  [32621, 32768, 0],
  [20708, 32768, 0],
  [8127, 32768, 0]
];

pub const DEFAULT_SKIP_CDF: [[u32; 3]; SKIP_CONTEXTS] = [
  [ 31671, 32768, 0 ],
  [ 16515, 32768, 0 ],
  [ 4576, 32768, 0 ]
];

pub const DEFAULT_COMP_REF_CDF: [[[u32; 3]; FWD_REFS - 1]; REF_CONTEXTS] = [
  [ [ 4946, 32768, 0 ],
    [ 9468, 32768, 0 ],
    [ 1503, 32768, 0 ] ],
  [ [ 19891, 32768, 0 ],
...
pub const DEFAULT_COMP_BWD_REF_CDF: [[[u32; 3]; BWD_REFS - 1]; REF_CONTEXTS] = [
  [ [  2235, 32768, 0 ], [  1423, 32768, 0 ] ],
  [ [ 17182, 32768, 0 ], [ 15175, 32768, 0 ] ],
  [ [ 30606, 32768, 0 ], [ 30489, 32768, 0 ] ]
];

pub const DEFAULT_SINGLE_REF_CDF: [[[u32; 3]; SINGLE_REFS - 1]; REF_CONTEXTS] = [
...
pub const DEFAULT_COMPOUND_MODE_CDF: [[u32; COMPOUND_MODES + 1]; COMPOUND_MODE_CONTEXTS] = [
...
pub const DEFAULT_INTERP_FILTER_CDF: [[u32; INTERP_FILTERS + 1]; INTERP_FILTER_CONTEXTS] = [
  [ 31935, 32720, 32768, 0 ],
  [ 5568, 32719, 32768, 0 ],
  [ 422, 2938, 32768, 0 ],
  [ 28244, 32608, 32768, 0 ],
...
pub const DEFAULT_MOTION_MODE_CDF: [[u32; MOTION_MODES + 1]; BLOCK_SIZES] = [
  [ 10923, 21845, 32768, 0 ],
  [ 10923, 21845, 32768, 0 ],
  [ 10923, 21845, 32768, 0 ],
  [ 7651, 24760, 32768, 0 ],
...
pub const DEFAULT_MV_JOINT_CDF: [u32; MV_JOINTS + 1] = [
  4096, 11264, 19328, 32768, 0
];

pub const DEFAULT_MV_CLASS_CDF: [[u32; MV_CLASSES + 1]; 2] = [
...
pub const DEFAULT_MV_CLASS0_FR_CDF: [[[u32; MV_JOINTS + 1]; CLASS0_SIZE]; 2] = [
...
pub const DEFAULT_MV_FR_CDF: [[u32; MV_JOINTS + 1]; 2] = [
  [ 8192, 17408, 21248, 32768, 0 ],
  [ 8192, 17408, 21248, 32768, 0 ],
];

pub const DEFAULT_PALETTE_Y_SIZE_CDF: [[u32; PALETTE_SIZES + 1]; PALETTE_BLOCK_SIZE_CONTEXTS] = [
...
pub const DEFAULT_PALETTE_UV_SIZE_CDF: [[u32; PALETTE_SIZES + 1]; PALETTE_BLOCK_SIZE_CONTEXTS] = [
...
pub const DEFAULT_PALETTE_SIZE_2_Y_COLOR_CDF: [[u32; 3]; PALETTE_COLOR_CONTEXTS] = [
  [ 28710, 32768, 0 ],
  [ 16384, 32768, 0 ],
  [ 10553, 32768, 0 ],
  [ 27036, 32768, 0 ],
...
pub const DEFAULT_PALETTE_SIZE_3_Y_COLOR_CDF: [[u32; 4]; PALETTE_COLOR_CONTEXTS] = [
  [ 27877, 30490, 32768, 0 ],
  [ 11532, 25697, 32768, 0 ],
  [ 6544, 30234, 32768, 0 ],
  [ 23018, 28072, 32768, 0 ],
...
pub const DEFAULT_PALETTE_SIZE_4_Y_COLOR_CDF: [[u32; 5]; PALETTE_COLOR_CONTEXTS] = [
...
pub const DEFAULT_PALETTE_SIZE_5_Y_COLOR_CDF: [[u32; 6]; PALETTE_COLOR_CONTEXTS] = [
...
pub const DEFAULT_PALETTE_SIZE_6_Y_COLOR_CDF: [[u32; 7]; PALETTE_COLOR_CONTEXTS] = [
...
pub const DEFAULT_PALETTE_SIZE_7_Y_COLOR_CDF: [[u32; 8]; PALETTE_COLOR_CONTEXTS] = [
...
pub const DEFAULT_PALETTE_SIZE_8_Y_COLOR_CDF: [[u32; 9]; PALETTE_COLOR_CONTEXTS] = [
...
pub const DEFAULT_PALETTE_SIZE_2_UV_COLOR_CDF: [[u32; 3]; PALETTE_COLOR_CONTEXTS] = [
  [ 29089, 32768, 0 ],
  [ 16384, 32768, 0 ],
  [ 8713, 32768, 0 ],
  [ 29257, 32768, 0 ],
...
pub const DEFAULT_PALETTE_SIZE_3_UV_COLOR_CDF: [[u32; 4]; PALETTE_COLOR_CONTEXTS] = [
  [ 25257, 29145, 32768, 0 ],
  [ 12287, 27293, 32768, 0 ],
  [ 7033, 27960, 32768, 0 ],
  [ 20145, 25405, 32768, 0 ],
...
pub const DEFAULT_PALETTE_SIZE_4_UV_COLOR_CDF: [[u32; 5]; PALETTE_COLOR_CONTEXTS] = [
...
pub const DEFAULT_PALETTE_SIZE_5_UV_COLOR_CDF: [[u32; 6]; PALETTE_COLOR_CONTEXTS] = [
...
pub const DEFAULT_PALETTE_SIZE_6_UV_COLOR_CDF: [[u32; 7]; PALETTE_COLOR_CONTEXTS] = [
...
pub const DEFAULT_PALETTE_SIZE_7_UV_COLOR_CDF: [[u32; 8]; PALETTE_COLOR_CONTEXTS] = [
...
pub const DEFAULT_PALETTE_SIZE_8_UV_COLOR_CDF: [[u32; 9]; PALETTE_COLOR_CONTEXTS] = [
...
pub const DEFAULT_PALETTE_Y_MODE_CDF: [[[u32; 3]; PALETTE_Y_MODE_CONTEXTS]; PALETTE_BLOCK_SIZE_CONTEXTS] = [
...
pub const DEFAULT_PALETTE_UV_MODE_CDF: [[u32; 3]; PALETTE_UV_MODE_CONTEXTS] = [
  [ 32461, 32768, 0 ], [ 21488, 32768, 0 ]
];

pub const DEFAULT_DELTA_Q_CDF: [u32; DELTA_Q_SMALL + 2] = [
  28160, 32120, 32677, 32768, 0
];

pub const DEFAULT_DELTA_LF_CDF: [u32; DELTA_LF_SMALL + 2] = [
  28160, 32120, 32677, 32768, 0
];

pub const DEFAULT_INTRA_TX_TYPE_SET1_CDF: [[[u32; 8]; INTRA_MODES]; 2] = [
    [
...
pub const DEFAULT_INTRA_TX_TYPE_SET2_CDF: [[[u32; 6]; INTRA_MODES]; 3] = [
    [
...
pub const DEFAULT_INTER_TX_TYPE_SET1_CDF: [[u32; 17]; 2] = [
...
pub const DEFAULT_INTER_TX_TYPE_SET2_CDF: [u32; 13] = [
    770, 2421, 5225, 12907, 15819, 18927, 21561, 24089, 26595,
    28526, 30529, 32768, 0
];

pub const DEFAULT_INTER_TX_TYPE_SET3_CDF: [[u32; 3]; 4] = [
    [ 16384, 32768, 0 ],
    [ 4167, 32768, 0 ],
    [ 1998, 32768, 0 ],
    [ 748, 32768, 0 ]
...
pub const DEFAULT_COMPOUND_IDX_CDF: [[u32; 3]; COMPOUND_IDX_CONTEXTS] = [
  [ 18244, 32768, 0 ],
  [ 12865, 32768, 0 ],
  [ 7053,  32768, 0 ],
  [ 13259, 32768, 0 ],
...
pub const DEFAULT_COMP_GROUP_IDX_CDF: [[u32; 3]; COMP_GROUP_IDX_CONTEXTS] = [
  [ 26607, 32768, 0 ],
  [ 22891, 32768, 0 ],
  [ 18840, 32768, 0 ],
  [ 24594, 32768, 0 ],
...
pub const DEFAULT_COMPOUND_TYPE_CDF: [[u32; COMPOUND_TYPES + 1]; BLOCK_SIZES] = [
  [ 16384, 32768, 0 ],
  [ 16384, 32768, 0 ],
  [ 16384, 32768, 0 ],
  [ 23431, 32768, 0 ],
...
pub const DEFAULT_INTER_INTRA_CDF: [[u32; 3]; BLOCK_SIZE_GROUPS - 1] = [
  [ 26887, 32768, 0],
  [ 27597, 32768, 0],
  [ 30237, 32768, 0]
];

pub const DEFAULT_INTER_INTRA_MODE_CDF: [[u32; INTERINTRA_MODES + 1]; BLOCK_SIZE_GROUPS - 1] = [
  [ 1875, 11082, 27332, 32768, 0 ],
  [ 2473, 9996, 26388, 32768, 0 ],
  [ 4238, 11537, 25926, 32768, 0 ]
];

pub const DEFAULT_WEDGE_INDEX_CDF: [[u32; 16+1]; BLOCK_SIZES] = [
...
pub const DEFAULT_WEDGE_INTER_INTRA_CDF: [[u32; 3]; BLOCK_SIZES] = [
  [ 16384, 32768, 0 ],
  [ 16384, 32768, 0 ],
  [ 16384, 32768, 0 ],
  [ 20036, 32768, 0 ],
...
pub const DEFAULT_USE_OBMC_CDF: [[u32; 3]; BLOCK_SIZES] = [
  [ 16384, 32768, 0 ],
  [ 16384, 32768, 0 ],
  [ 16384, 32768, 0 ],
  [ 10437, 32768, 0 ],
...
pub const DEFAULT_COMP_REF_TYPE_CDF: [[u32; 3]; COMP_REF_TYPE_CONTEXTS] = [
  [ 1198, 32768, 0 ],
  [ 2070, 32768, 0 ],
  [ 9166, 32768, 0 ],
  [ 7499, 32768, 0 ],
...
pub const DEFAULT_UNI_COMP_REF_CDF: [[[u32; 3]; UNIDIR_COMP_REFS - 1]; REF_CONTEXTS] = [
  [ [ 5284, 32768, 0 ], [ 3865, 32768, 0 ], [ 3128, 32768, 0 ] ],
  [ [ 23152, 32768, 0 ], [ 14173, 32768, 0 ], [ 15270, 32768, 0 ] ],
  [ [ 31774, 32768, 0 ], [ 25120, 32768, 0 ], [ 26710, 32768, 0 ] ]
];

pub const DEFAULT_CFL_SIGN_CDF: [u32; CFL_JOINT_SIGNS + 1 ] = [
  1418, 2123, 13340, 18405, 26972, 28343, 32294, 32768, 0
];

pub const DEFAULT_CFL_ALPHA_CDF: [[u32; CFL_ALPHABET_SIZE + 1]; CFL_ALPHA_CONTEXTS] = [
...
pub const DEFAULT_USE_WIENER_CDF: [u32; 2 + 1] = [
  11570, 32768, 0
];

pub const DEFAULT_USE_SGRPROJ_CDF: [u32; 2 + 1] = [
  16855, 32768, 0
];

pub const DEFAULT_RESTORATION_TYPE_CDF: [u32; RESTORE_SWITCHABLE + 1] = [
  9413, 22581, 32768, 0
];

pub const DEFAULT_TXB_SKIP_CDF: [[[[u32; 3]; TXB_SKIP_CONTEXTS]; TX_SIZES]; COEFF_CDF_Q_CTXS] = [
  [
    [ [ 31849, 32768, 0 ],
      [ 5892, 32768, 0 ],
      [ 12112, 32768, 0 ],
...
pub const DEFAULT_EOB_PT_16_CDF: [[[[u32; 6]; 2]; PLANE_TYPES]; COEFF_CDF_Q_CTXS] = [
...
pub const DEFAULT_EOB_PT_32_CDF: [[[[u32; 7]; 2]; PLANE_TYPES]; COEFF_CDF_Q_CTXS] = [
...
pub const DEFAULT_EOB_PT_64_CDF: [[[[u32; 8]; 2]; PLANE_TYPES]; COEFF_CDF_Q_CTXS] = [
...
pub const DEFAULT_EOB_PT_128_CDF: [[[[u32; 9]; 2]; PLANE_TYPES]; COEFF_CDF_Q_CTXS] = [
...
pub const DEFAULT_EOB_PT_256_CDF: [[[[u32; 10]; 2]; PLANE_TYPES]; COEFF_CDF_Q_CTXS] = [
...
pub const DEFAULT_EOB_PT_512_CDF: [[[u32; 11]; PLANE_TYPES]; COEFF_CDF_Q_CTXS] = [
...
pub const DEFAULT_EOB_PT_1024_CDF: [[[u32; 12]; PLANE_TYPES]; COEFF_CDF_Q_CTXS] = [
...
pub const DEFAULT_EOB_EXTRA_CDF: [[[[[u32; 3]; EOB_COEF_CONTEXTS]; PLANE_TYPES]; TX_SIZES]; COEFF_CDF_Q_CTXS] = [
...
pub const DEFAULT_DC_SIGN_CDF: [[[[u32; 3]; DC_SIGN_CONTEXTS]; PLANE_TYPES]; COEFF_CDF_Q_CTXS] = [
  [ [
      [ 128 * 125, 32768, 0 ],
      [ 128 * 102, 32768, 0 ],
      [ 128 * 147, 32768, 0 ],
...
pub const DEFAULT_COEFF_BASE_EOB_CDF: [[[[[u32; 4]; SIG_COEF_CONTEXTS_EOB]; PLANE_TYPES]; TX_SIZES]; COEFF_CDF_Q_CTXS] = [
  [ [ [ [ 17837, 29055, 32768, 0 ],
        [ 29600, 31446, 32768, 0 ],
        [ 30844, 31878, 32768, 0 ],
        [ 24926, 28948, 32768, 0 ] ],
...
pub const DEFAULT_COEFF_BASE_CDF: [[[[[u32; 5]; SIG_COEF_CONTEXTS]; PLANE_TYPES]; TX_SIZES]; COEFF_CDF_Q_CTXS] = [
...
#[allow(clippy::large_const_arrays)]
pub const DEFAULT_COEFF_BR_CDF: [[[[[u32; BR_CDF_SIZE + 1]; LEVEL_CONTEXTS]; PLANE_TYPES]; TX_SIZES]; COEFF_CDF_Q_CTXS] = [
